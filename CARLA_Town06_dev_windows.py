...
ABSOLUTE_PATH = "D:\\Ford\\CarlaTables\\Tables\\"
DEBUG_LOG_PATH = "/home/carla/CarlaTable/log/"
DELTA_V_CAP = 1e-5
FREQUENCY = 2000 # HZ
DELTA_T = 1 / FREQUENCY
MAX_ELAPSED_SECONDS = 25
...
def loop(throttle:float, steer:float, ego_vehicle:Actor, spectator:Actor, \
    ...
    elapsed_seconds_zero = world.get_snapshot().elapsed_seconds
    elapsed_seconds = 0
    # vehicle_at_const_speed_counter = 0
#     while True:
...
#     print(vehicle_is_in_road(ego_vehicle, elapsed_seconds))
    while (elapsed_seconds <= 2) | (
            (elapsed_seconds <= MAX_ELAPSED_SECONDS) & vehicle_is_in_road(ego_vehicle)):
        ...
        elapsed_seconds = snapshot.elapsed_seconds - elapsed_seconds_zero
...
throttle = 0.0
steer = 0.0
for run in range(331, 341):
    ego_vehicle_spawn_point = Transform(Location(x=-272, y=-18, z=0.061494), \
                                    Rotation(pitch=0.000000, yaw=0.0, roll=0.000000))
